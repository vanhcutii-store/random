...
function empty(a) {
    return undefined === a || null == a || "" == a;
  }
  function getRandText(a) {
    return Array.isArray(a) ? a[Math.floor(Math.random() * a.length + 1) - 1] : a;
  }
  function showMessage(a, c, e) {
    (e || "" === sessionStorage.getItem("waifu-text") || null === sessionStorage.getItem("waifu-text")) && (Array.isArray(a) && (a = a[Math.floor(Math.random() * a.length + 1) - 1]), live2d_settings.showF12Message && console.log("[Message]", a.replace(/<[^<>]+>/g, "")), e && sessionStorage.setItem("waifu-text", a), $(".waifu-tips").stop(), $(".waifu-tips").html(a).fadeTo(200, 1), undefined === c && (c = 5E3), hideMessage(c));
  }
  ...
  function initModel(a, c) {
    ...
    "object" == (undefined === a ? "undefined" : _typeof(a)) ? loadTipsMessage(a) : $.ajax({
      cache: true,
  ...
  function loadModel(a) {
    var c = 1 < arguments.length && undefined !== arguments[1] ? arguments[1] : 0;
  ...
  function loadTipsMessage(a) {
    function c() {
      ...
      $.ajax({
        ...
        success: function (a) {
          ...
          showMessage("Đã đổi nhân vật", 3E3, true);
        }
      });
    }
    function e() {
      ...
      $.ajax({
        ...
        dataType: "json",
        success: function (l) {
          1 != l.textures.id || 1 != b && 0 != b ? showMessage(f.load_rand_textures[1], 3E3, true) : showMessage(f.load_rand_textures[0], 3E3, true);
        ...
        case "lwl12.com":
          $.getJSON("https://api.lwl12.com/hitokoto/v1?encode=realjson", function (a) {
            if (!empty(a.source)) {
              ...
              window.setTimeout(function () {
                showMessage(b + f.hitokoto_api_message["lwl12.com"][2], 3E3, true);
              }, 5E3);
            }
            showMessage(a.text, 5E3, true);
          });
          break;
        case "fghrsh.net":
          $.getJSON("https://api.fghrsh.net/hitokoto/rand/?encode=jsc&uid=3335", function (a) {
            if (!empty(a.source)) {
              ...
              window.setTimeout(function () {
                showMessage(b, 3E3, true);
              }, 5E3);
              showMessage(a.hitokoto, 5E3, true);
            }
          });
          break;
        case "jinrishici.com":
          $.ajax({
            ...
            success: function (a, b) {
              if (!empty(a.data.origin.title)) {
                ...
                window.setTimeout(function () {
                  showMessage(d, 3E3, true);
                }, 5E3);
              }
              showMessage(a.data.content, 5E3, true);
            }
          });
          break;
        default:
    ...
    $.each(a.seasons, function (a, b) {
      ...
      d.split("/")[0] <= a.getMonth() + 1 && a.getMonth() + 1 <= c.split("/")[0] && d.split("/")[1] <= a.getDate() && a.getDate() <= c.split("/")[1] && (b = getRandText(b.text), b = b.render({
        year: a.getFullYear()
      }), showMessage(b, 6E3, true));
    });
    live2d_settings.showF12OpenMsg && (re.toString = function () {
      return showMessage(getRandText(a.waifu.console_open_msg), 5E3, true), "";
    });
    live2d_settings.showCopyMessage && $(document).on("copy", function () {
      showMessage(getRandText(a.waifu.copy_message), 5E3, true);
    });
    $(".waifu-tool .fui-photo").click(function () {
      showMessage(getRandText(a.waifu.screenshot_message), 5E3, true);
      window.Live2D.captureName = live2d_settings.screenshotCaptureName;
      window.Live2D.captureFrame = true;
    });
    $(".waifu-tool .fui-cross").click(function () {
      sessionStorage.setItem("waifu-dsiplay", "none");
      showMessage(getRandText(a.waifu.hidden_message), 1300, true);
      window.setTimeout(function () {
        $(".waifu").hide();
      }, 1300);
    });
    ...
    var f = a.waifu;
    live2d_settings.showHitokoto && (window.getActed = false, window.hitokotoTimer = 0, window.hitokotoInterval = false, $(document).mousemove(function (a) {
      getActed = true;
    }).keydown(function () {
      getActed = true;
    }), setInterval(function () {
      getActed ? (getActed = hitokotoInterval = false, window.clearInterval(hitokotoTimer)) : hitokotoInterval || (hitokotoInterval = true, hitokotoTimer = window.setInterval(h, 3E4));
    }, 1E3));
  ...
  live2d_settings.screenshotCaptureName = "ScreenWithWaifu.png";
  String.prototype.render = function (a) {
    return this.replace(/(\\)?\{([^\{\}\\]+)(\\)?\}/g, function (c, e, g, h) {
      ...
      h = a;
      c = 0;
      for (e = g.length; c < e; ++c) if (k = g[c], undefined === (h = h[k]) || null === h) return "";
      return h;
    });
  };
  var re = /x/;
